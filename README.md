...
2. **Judge** — each of the 3 answers is independently checked by 3 different models — **GPT-OSS-20B (via Groq), Gemini, and Mistral** — each verifying whether that specific answer looks correct or hallucinated. The model that generates answers never judges its own work.
...
*Note: these numbers were measured with Claude as the judge/consistency-checker model (the setup used at evaluation time). The app has since switched to GPT-OSS-20B via Groq in that role; results have not yet been re-measured against the new configuration.*
